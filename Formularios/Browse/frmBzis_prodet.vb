Public Class frmBzis_prodet

    Private moFormulario As frmFzis_prodet
    ...
    Private Sub frmBrowse_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        AddHandler btnProcesar.MouseDown, AddressOf Imagen_Click
        ...
        loFk.numtra = miNumTra
        If loFk.GetPK() = sOk_ Then
            lblNombreTabla.Text = "Transacción: " & miNumTra.ToString & " " & loFk.abreviado
        End If
        loFk = Nothing
        mbabrirform = False
        LPCargarDatos()
    ...
    Private Sub LPCargarDatos()
        ...
        lsSQL = GFsGeneraSQL("frmBzis_prodet")
        Dim lsConcatFiltro As String = "numord, nummod, modelo, formact, start, ending"
        ...
        End If
        lsSQL = lsSQL.Replace("&numtra", miNumTra.ToString)
    ...
    Private Sub LPLocalizaRegistro(ByVal psCodigo As String)
        ...
        For Each row As DataGridViewRow In DataGridView1.Rows
            If row.Cells("numord").Value.ToString = psCodigo Then
    ...
    Private Sub Botones_Click(sender As Object, e As EventArgs)
        ...
        Select Case CType(sender, Button).AccessibleName
            Case sAGREGAR_
                moFormulario = New frmFzis_prodet
            ...
            Case Else
                ...
                '---------------------------------------------------------------------------------------------
                Dim lsCodigo As String = DataGridView1.Item("numord", DataGridView1.CurrentRow.Index).Value.ToString
                ...
                Try
                    loDatos.numord = CInt(lsCodigo)
                    If loDatos.GetPK = sOk_ Then
                        moFormulario = New frmFzis_prodet
                        moFormulario.AccessibleName = "Transacción: " & loDatos.numtra
                        moFormulario.Tag = CType(sender, Button).AccessibleName
                        moFormulario.entidad = loDatos
                        GPCargar(moFormulario)
                        msLocalizar = moFormulario.AccessibleName
                ...
                Catch ex As Exception
                    GFsAvisar("Error en Browse", sError_, "No existe datos para la transacción [" & loDatos.numtra & " / Orden:" & loDatos.numord & "]" & ControlChars.CrLf & ex.Message)
                End Try
        End Select
        loDatos.CerrarConexion()
        loDatos = Nothing
    ...
    Private Sub btnProcesar_MouseMove(sender As Object, e As EventArgs) Handles btnProcesar.MouseMove, btnProcesar.MouseHover
        btnProcesar.Image = My.Resources.Resources.icons8_processing_48
        btnProcesar.Text = ""
    End Sub

    Private Sub btnProcesar_MouseLeave(sender As Object, e As EventArgs) Handles btnProcesar.MouseLeave
        btnProcesar.Image = My.Resources.Resources.icons8_processing_32
        btnProcesar.Text = btnProcesar.Tag.ToString
    End Sub
    ...
    Overloads Sub Imagen_Click(sender As Object, Optional e As MouseEventArgs = Nothing)
        Select Case CType(sender, Button).Name
            ...
            Case "btnConsultar"
                ...
                btnConsultar.Text = ""
            Case "btnProcesar"
                btnProcesar.Image = My.Resources.Resources.icons8_color_processing_48
                btnProcesar.Text = ""
            Case "btnAuditoria"
    ...
    Private Sub btnProcesar_Click(sender As Object, e As EventArgs) Handles btnProcesar.Click
        If DataGridView1.CurrentRow Is Nothing Then Exit Sub
        Dim loZIS_PRODET As New Ezis_prodet
        loZIS_PRODET.numtra = miNumTra
        loZIS_PRODET.numord = CInt(DataGridView1.Item("numord", DataGridView1.CurrentRow.Index).Value.ToString)
        If loZIS_PRODET.GetPK = sOk_ Then
            Dim loProcesar As New frmFzis_procesar_prodet
            loProcesar.NumTra = loZIS_PRODET.numtra
            loProcesar.NumOrd = loZIS_PRODET.numord
            GPCargar(loProcesar)
        End If
